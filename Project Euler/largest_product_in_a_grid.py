...
if __name__ == "__main__":
    ...
    for row in range(len(arr)):
        curr_max_product = 0
        for i in range(len(arr[row])-4):
            curr_max_product = max(
                curr_max_product, get_multiply_elements(arr[row][i:i+4]))
        global_max_product = max(global_max_product, curr_max_product)

    # Vertical
    for col in range(len(arr)):
        curr_max_product = 0
        sub_arr = [arr[row][col] for row in range(len(arr[col]))]
        for i in range(len(sub_arr)-4):
            curr_max_product = max(
                curr_max_product, get_multiply_elements(sub_arr[i:i+4]))
        global_max_product = max(global_max_product, curr_max_product)

    # Diagonal TL-BR
    for row in range(len(arr) - 3):
        for col in range(len(arr[row]) - 3):
            product = 1
            for k in range(4):
                product *= arr[row+k][col+k]
            global_max_product = max(product, global_max_product)
